...
call ResetGrid arrGrid, PossibleBlocks
call PrintGrid arrGrid
*/
...
do while \Finished
  ...
  if nexty = 0 | nextx = 0 | nexty = height + 1 | nextx = width + 1 then Finished = .True
  else if arrGrid[nexty, nextx] = '#' then do 
    ...
    select case arrGrid[guard~y, guard~x]
      when '^' then NOP
      ...
      otherwise arrGrid[guard~y, guard~x] = '+'
    end        
  end
end  
return visited
...
::ROUTINE ResetGrid
use arg arrGrid, possibleblocks
  do i = 1 to possibleblocks~Y~items
    arrGrid[possibleblocks~Y[i], possibleblocks~X[i]] = '.'
  end
